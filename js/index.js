...
$(document).ready(function() {
	...
	function invideo() {
		...
		setTimeout(function() {
			$.getJSON("http://47.103.41.41:8086/api/Video/GetVideoInfoList", {
				...
				for (var i = 0; i < data.data.rows.length; i++) {
					var htmlStr = ``;
					var Hour = ((data.data.rows[i].F_Hour + '').length < 2) ? "0" + data.data.rows[i].F_Hour : data.data.rows[i]
						.F_Hour;
					var Minute = ((data.data.rows[i].F_Minute + '').length < 2) ? "0" + data.data.rows[i].F_Minute : data.data.rows[
						i].F_Minute;
					var Second = ((data.data.rows[i].F_Second + '').length < 2) ? "0" + data.data.rows[i].F_Second : data.data.rows[
	...
	$(".table_tbody").on("click", ".btn-default", function() { //编辑
		...
		$.get('http://47.103.41.41:8086/api/Video/GetVideoInfoById', {
			F_Id: F_Id
		}, function(data) {
			// console.log(data);
	...
	$(".table_tbody").on("click", ".btn-danger", function() { //删除
		var F_Id = $(this).parents("tr").attr('id');
		$.get('http://47.103.41.41:8086/api/Video/DeleteVideoInfo', {
			F_Id: F_Id,
		}, function(data) {
			console.log(data);
		});
